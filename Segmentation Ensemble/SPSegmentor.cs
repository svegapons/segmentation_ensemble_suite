...
{
    public class SPSegmentor: NonHierarchical
    {
    ...
    public abstract class SegmentationIndex
    {
        ...
        //Como en esta variante muy mal programada en Set esta toda la info..incluso las segmentaciones...no hace falta mas nada.
        //Aqui lo q se devuelve es el arreglo con los valores de la evaluacion de cada segmentacion por el indice.
        protected abstract double[] Evaluate(Set set);
        ...
        public void EvaluateSegmentations(Set set)
        {
            Evaluations = Evaluate(set);
    ...
    {
        protected override double[]  Evaluate(Set set)
        {
            double[] result = new double[set.Segmentations.Count];
            int perim = 0;
            double compact = 0;
            int cont = 0;
            foreach (Segmentation  seg in set.Segmentations)
            {
                int sp = 0;
                foreach (int cluster in seg.Clusters.Keys)
                {
                    ...
                    for (int i = 0; i < seg.Clusters[cluster].Count; i++)
                    {
                        ...
                        perim += (int)set.Elements[sp][set.Attributes.ValuesCount - 1];
                        for (int j = i + 1; j < seg.Clusters[cluster].Count; j++)
                        {
                            perim -= 2 * set.SPGraph.AdjMatrix[sp, seg.Clusters[cluster][j]];
                        }
                    }
    ...
    public class ConnectivityIndex : ShapeIndex
    {
        protected override double[] Evaluate(Set set)
        {
            ...
            foreach (Segmentation seg in set.Segmentations)
            {
                for (int i = 0; i < set.SPGraph.AdjList.Length; i++)
                {
                    for (int j = 0; j < nc; j++)
                    {
                        if (j < set.SPGraph.AdjList[i].Count && seg.Labels[i] != seg.Labels[set.SPGraph.AdjList[i][j]])
